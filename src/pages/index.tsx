...
const Home: NextPage = ({
  ...
  return (
    <>
      ...
      <main className='min-h-screen max-w-screen bg-[#EAECF1] flex flex-col justify-center items-center relative'>
        <div className='w-full flex justify-end items-center px-4 py-4'>
          <button
            onClick={session ? () => signOut() : () => signIn('discord')}
            className='block bg-[#5457B6] uppercase self-end rounded-md px-4 py-2 text-white hover:opacity-50 '
          >
            {session ? 'Logout' : 'Login with Discord'}
          </button>
        </div>
        {comments.map((comment) => {
          const relativeDate = howLongHasItBeen(comment.updatedAt);
          if (comment.replies.length > 0) {
            return (
              <div
                key={comment.id}
                className=' w-[94%] sm:w-[80%] md:w-[70%] lg:w-[60%] xl:w-[50%]'
              >
                <Comment
                  showReply={showReply}
                ...
                <div className='pl-[1rem] sm:pl-[2.1rem] md:pl-[2.3rem] mt-5 '>
                  <div className='w-full border-l-2  border-x-[#C3C4EF] flex justify-evenly items-end flex-col  '>
                    {comment.replies.map((reply) => {
                      const relativeDate = howLongHasItBeen(reply.updatedAt);
                      return (
                        <Comment
                          showReply={showReply}
                          setShowReply={setShowReply}
